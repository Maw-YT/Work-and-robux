...
let avoidanceIntervalId = null;
let avoidanceCheckInterval = 250; // Default interval
...
export const aiCursorDraggable = {
  ...
  init() {
    ...
    this.startAvoidanceCheck(); // Start the interval check
  },

  startAvoidanceCheck() {
    if (avoidanceIntervalId) {
      clearInterval(avoidanceIntervalId);
    }
    avoidanceCheckInterval = gameState.lowPerformanceMode ? 500 : 250; // Adjust interval based on potato mode
    avoidanceIntervalId = setInterval(() => {
      this.checkCursorAvoidance();
  ...
  checkCursorAvoidance() {
    // Skip check if in potato mode to save performance
    if (gameState.lowPerformanceMode) return;
    ...
    }).catch(err => console.error("Error loading aiCursor in checkCursorAvoidance:", err));
  },

  applyCursorAvoidance(cursorElement) {
     // Skip check if in potato mode
     if (gameState.lowPerformanceMode) return;
...
// Update avoidance interval if potato mode changes
// This needs a way to be notified. Let's modify the console command to call a function here.
export function updateAvoidanceInterval() {
    if (aiCursorDraggable) { // Check if initialized
        aiCursorDraggable.stopAvoidanceCheck();
        aiCursorDraggable.startAvoidanceCheck();
    }
}