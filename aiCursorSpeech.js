...
export const aiCursorSpeech = {
  enabled: true, // Flag to control speech bubble visibility
  ...
  createSpeechBubble(message, cursor, duration = 4000) {
    if (!this.enabled) return null; // Don't create if disabled
  ...
  speak(cursor, state, customMessage = null) {
    if (!this.enabled || !cursor) return; // Don't speak if disabled
  ...
  speakAll(customMessage = null) {
    if (!this.enabled) return; // Don't speak if disabled
