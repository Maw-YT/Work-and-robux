...
export const aiCursor = {
  ...
  animateAdditionalCursor(cursorData, targetX, targetY, callback = null) {
    ...
    // Shorter animation for additional cursors
    ...
    const animate = (currentTime) => {
      ...
      // Use cubic easing for more realistic movement
      const easedProgress = this.easeInOutCubic(progress);
      
      const currentX = startX + (targetX - startX) * easedProgress;
      const currentY = startY + (targetY - startY) * easedProgress;
      
      // Add more dynamic curve to movement
      const curveAmplitude = 10; // Increased amplitude for more pronounced curve
      const dynamicCurve = Math.sin(progress * Math.PI * (1 + Math.random() * 0.2)) * curveAmplitude * (Math.random() > 0.5 ? 1 : -1);
      
      // Apply position with curve
      cursor.style.left = `${currentX}px`;
      cursor.style.top = `${currentY + dynamicCurve}px`;
      
      // Add slight rotation during movement
      const rotationAngle = Math.sin(progress * Math.PI * 2) * 8; // Increased rotation
      cursor.style.transform = `rotate(${rotationAngle}deg) scale(${0.9 + progress * 0.2})`; // Scale changes during movement
