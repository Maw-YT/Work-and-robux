...
export const aiCursor = {
  ...
  baseCpuSpeed: 300, // Store the base speed before potato mode adjustments
  isMoving: false,
  ...
  animateCursorMovement(startX, startY, endX, endY, callback) {
    const duration = gameState.lowPerformanceMode ? 500 : 400; // Slower movement in potato mode
    const startTime = performance.now();

    // Remove transition temporarily for animation
    if (this.cursor) this.cursor.style.transition = 'none';

    // Add slight hover effect during movement (skip in potato mode)
    if (this.cursor && !gameState.lowPerformanceMode) this.cursor.classList.add('active');

    const animate = (currentTime) => {
      if (!this.cursor) return; // Check if cursor exists
      ...
      // Simplify movement in potato mode (no curve or rotation)
      if (gameState.lowPerformanceMode) {
        this.cursor.style.left = `${currentX}px`;
        this.cursor.style.top = `${currentY}px`;
        this.cursor.style.transform = 'scale(1)'; // Ensure no leftover transform
      } else {
      ...
      if (progress < 1) {
        ...
        // Movement complete
        if (this.cursor && !gameState.lowPerformanceMode) this.cursor.classList.remove('active'); // Remove active class if not in potato mode
  ...
  performClickAnimation(targetButton, cursorData = null) {
     // ... (keep existing sound and click logic) ...

    const cursor = cursorData ? cursorData.element : this.cursor;
    if (!cursor) return; // Check if cursor exists

    // Simplify or skip animation in potato mode
    if (!gameState.lowPerformanceMode) {
        ...
        targetButton.classList.add('ai-click-animation');
         ...
         cursor.style.transform = 'scale(1)'; // No transform change
         // Skip adding 'ai-click-animation' class to target button
    }
    ...
    setTimeout(() => {
        if (!cursor) return; // Check again
        if (!gameState.lowPerformanceMode) {
            cursor.classList.remove('active');
            cursor.style.transform = 'rotate(0deg) scale(1)';
             if (targetButton) targetButton.classList.remove('ai-click-animation');
        } else {
            cursor.style.transform = 'scale(1)'; // Ensure reset
        }

      // Reset isMoving flag
      if (!cursorData) {
        this.isMoving = false;
      } else if (cursorData) { // Ensure cursorData exists
        cursorData.isMoving = false;
      }
    }, gameState.lowPerformanceMode ? 100 : 400); // Shorter timeout in potato mode
  },
  ...
  startCPUMode(customSpeed = null) {
    ...
    let speedSetting = customSpeed; // The slider-like value (100-900)

    if (customSpeed === null) {
        // Reset to default if no custom speed provided
        this.baseCpuSpeed = 300; // Default interval speed
        speedSetting = 600; // Default slider value
    } else {
        // Calculate base speed from the custom setting if needed (assuming customSpeed is 100-900)
        this.baseCpuSpeed = 1000 - speedSetting;
    }

    // Adjust actual interval speed based on potato mode
    this.cpuSpeed = gameState.lowPerformanceMode ? this.baseCpuSpeed * 2 : this.baseCpuSpeed;

    // Update UI if customSpeed was null (resetting)
    if (customSpeed === null) {
        const cpuSpeedSlider = document.getElementById('cpuSpeedSlider');
        const cpuSpeedDisplay = document.getElementById('cpuSpeedDisplay');
        if (cpuSpeedSlider) cpuSpeedSlider.value = 600;
        if (cpuSpeedDisplay) cpuSpeedDisplay.textContent = '60%';
    }
  ...
  stopCPUMode() {
    ...
    if (this.cpuIntervalId) {
      ...
      // Reset to default base speed
      this.baseCpuSpeed = 300;
      this.cpuSpeed = this.baseCpuSpeed; // Use base speed
  ...
  setCPUSpeed(speed) { // speed is the slider value 100-900
    this.baseCpuSpeed = 1000 - speed; // Calculate base interval from slider value
    this.cpuSpeed = gameState.lowPerformanceMode ? this.baseCpuSpeed * 2 : this.baseCpuSpeed; // Adjust based on potato mode
    ...
    if (this.cpuIntervalId) {
      this.stopCPUMode();
      this.startCPUMode(speed); // Restart with the slider value
    }
  },

  getCPUSpeedPercentage() {
    // Calculate percentage based on BASE speed, not the potentially adjusted cpuSpeed
    const sliderValue = 1000 - this.baseCpuSpeed;
  ...
  animateAdditionalCursor(cursorData, targetX, targetY, callback = null) {
    ...
    const duration = gameState.lowPerformanceMode ? 600 : (300 + Math.random() * 200); // Slower in potato mode
    const startTime = performance.now();

    cursor.style.transition = 'none';
    if (!gameState.lowPerformanceMode) cursor.classList.add('active'); // Skip active state in potato mode
    ...
    const animate = (currentTime) => {
      ...
      cursor.style.transform = `scale(1)`; // Keep simple transform

      if (progress < 1) {
        requestAnimationFrame(animate);
      } else {
        if (!cursor || !cursor.parentNode) return;
        if (!gameState.lowPerformanceMode) cursor.classList.remove('active');
