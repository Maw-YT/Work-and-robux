...
export function setupEventListeners() {
  ...
  const exitButtons = document.querySelectorAll('#exitHomeBtn');
  exitButtons.forEach(button => {
    button.addEventListener('click', () => {
      window.location.href = 'https://maw-yt.github.io/home-games/';
    });
  });
  ...
  const themeStates = ['default', 'dark', 'evil', 'win95'];
  ...
  function getCurrentThemeIndex() {
    ...
    const index = themeStates.indexOf(savedTheme);
    return index === -1 ? 0 : index;
  }

  function applyTheme(themeIndex) {
    const newTheme = themeStates[themeIndex];
    document.body.classList.remove('dark-mode', 'evil-mode', 'win95-mode');

    if (newTheme === 'dark') {
      document.body.classList.add('dark-mode');
    } else if (newTheme === 'evil') {
    ...
    if (newTheme === 'win95') {
      playerCursor.setSystemCursorVisibility(true);
    } else {
      playerCursor.setSystemCursorVisibility(!playerCursor.isVisible);
    }
  }
  ...
  shopSellButtons.forEach(button => {
    button.addEventListener('click', (e) => {
      ...
      if (itemDetails.owned) {
        const sellPrice = Math.floor(itemDetails.sellPrice || itemDetails.basePrice);
