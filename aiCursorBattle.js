...
export const aiCursorBattle = {
  ...
  createDamageOverlay() {
    ...
    // Add class for potato mode styling
    if (gameState.lowPerformanceMode) {
        this.damageOverlay.classList.add('potato-mode-overlay');
    }
  ...
  flashDamageOverlay(amount) {
    // Skip flashing in potato mode
    if (gameState.lowPerformanceMode || !this.damageOverlay) return;
    ...
    this.damageOverlay.style.transition = 'opacity 0.2s ease'; // Ensure transition is set
  ...
  playerDeath() {
    ...
    // Simpler overlay in potato mode
    if (this.damageOverlay) {
        this.damageOverlay.style.backgroundColor = gameState.lowPerformanceMode ? 'rgba(0, 0, 0, 0.9)' : 'rgba(0, 0, 0, 0.98)';
        this.damageOverlay.style.opacity = '1';
        this.damageOverlay.style.transition = gameState.lowPerformanceMode ? 'none' : 'opacity 2s ease-in';
    }
  ...
  revivePlayer() {
    ...
    // Fade out black overlay (faster/no fade in potato mode)
     if (this.damageOverlay) {
        this.damageOverlay.style.transition = gameState.lowPerformanceMode ? 'none' : 'opacity 1s ease-out';
        this.damageOverlay.style.opacity = '0';
    }
